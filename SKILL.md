...
Coordinate configured agent CLIs inside the current Orca worktree. Use the bundled script for deterministic registry updates, terminal discovery, lazy terminal creation, and dispatch.
...
The registry file is `.orchestrater/agents.json`. Keep it project-local and suitable for commit unless the user says otherwise.
